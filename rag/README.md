# KFinLaw RAG 벤치마크 (한국 금융 법령 검색·답변)

금융 법령 질의응답에 RAG[[1]](#ref1)를 도입하려면 청킹·임베딩·검색기·리랭커·증강 등 수많은 설계 선택이 따르지만, **한국어 법령 도메인에서 무엇이 실제로 효과적인지에 대한 정량 근거는 빈약하다.** 영어·일반 도메인의 통념(예: 그래프 RAG·가설질의 증강이 검색을 개선한다, 큰 모델이 더 낫다)을 검증 없이 들여오면 비용만 늘고 성능은 오히려 떨어질 수 있다. 본 연구는 한국 금융 법령을 테스트베드로 **재현 가능한 통제 실험으로 기법별 효과를 정량 측정**해, 사내 법령·규정 검색 서비스 설계의 근거를 제공한다.

> 이 문서는 벤치마크(연구) 상세다. 프로젝트 개요·도구(MCP/CLI) 사용법은 [루트 README](../README.md).
...
>
> **→ 권장:** 조 청킹 + KURE-v1 + 하이브리드 + 리랭커를 기본값으로, 증강기법은 도입하지 말 것. 상세 권고는 [§7](#7-conclusion-and-recommendations).

한국 금융 법령 위에서 RAG 검색·답변 기법을 통제 실험으로 비교한다. 파이프라인 구성요소(§1)와 벤치마크 데이터(§2)를 정의하고, 평가 방법론(§3)에 따라 검색 평가(§4)와 답변 평가(§5)를 수행하며, 재현 절차(§6)를 제공한다.
...
파이프라인 단계별 선택지를 한 번에 하나씩만 바꾸며(일변수 격리) 비교했다. 아래 표에 단계별 비교 선택지를 정리한다(답변 생성은 답변모델 5종을 비교, [§5](#5-answer-evaluation)). 평가에 쓰는 생성모델(골드셋 생성, §2)·심사모델(judge, §3)은 파이프라인 구성요소가 아니라 각 해당 절에서 다룬다.
...
골드셋은 성격이 다른 세 벤치마크로 이뤄진다. 기본은 코퍼스에서 만든 **Lexical Benchmark**(240문항)이고, 어휘격차(질문이 조문과 다른 단어로 묻는 정도)를 키운 **Semantic Benchmark**와 위치 질의용 **Locator Benchmark**가 더해진다. 세 벤치마크 모두 정답을 조문·별표 식별자로 고정하는 동일한 스키마를 따르며, 평가 결과는 [§4](#4-retrieval-evaluation)에서 다룬다.
...
### 4.3 Lexical-Gap Robustness

**검색기 우열이 질의 표현(어휘중첩 ↔ 어휘격차)에 따라 뒤바뀌는가?** 질문이 조문 용어를 그대로 쓰면 검색어와 조문이 겹치는 정도(어휘중첩)가 크고, 같은 뜻을 일상어로 바꿔 물으면 둘 사이의 거리(어휘격차)가 크다. §2의 Lexical Benchmark는 조문에서 질문을 만들고 BM25 일관성 필터로 거르므로 어휘중첩이 큰 쪽으로 치우친다. 그렇다면 BM25 우위나 'HyPE·HyDE가 효과 없다'는 결과도 이런 치우침 때문에 나온 것일 수 있다. 이것이 사실인지 확인하려고 어휘격차를 키운 **Semantic Benchmark**를 만들었다(생성·필터는 §2).
...
- **질의 표현에 따라 최적 검색기가 달라지므로 상황에 맞게 골라 써야 한다.** 구체적 권고는 [§7](#7-conclusion-and-recommendations)에 정리했다.

### 4.4 Locator Queries

**법령명과 조번호로 특정 조문을 직접 지정하는 '위치 질의'를 RAG 검색으로 풀 수 있는가?** "○○법 제5조", "제5조부터 제8조까지"처럼 사용자가 **답이 아니라 조문 위치를 이미 알고** 찾는 known-item 질의다(유형 예시는 [§2.4](#24-골드셋)). 정보요구형(Lexical·Semantic)과 달리 정답이 내용이 아니라 위치로 정해지며, RAG 검색이 정확한 식별자를 얼마나 잘 다루는지 살펴본다.
...
- **전체 recall이 낮다**(최고 0.66). 위치 질의는 RAG 검색보다 **구조적 조회**(법령명과 조번호를 파싱해 조문을 바로 가져오는 방식)가 정답이며, 이것이 [MCP/CLI](../README.md) 구축의 직접적 근거다.
...
> **한계.** 통제된 소규모 코퍼스(32법령)라 절대 수치보다 기법 간 상대 비교에 무게를 둔다. 답변 평가는 심사모델 기반이라 법률가 검수·일치계수(κ) 같은 인적 검증이 빠졌고(ARES[[17]](#ref17)·KBL[[18]](#ref18) 관행), 골드셋도 단일 생성모델(Mistral)에 의존한다. 보강 방향은 [§7](#7-conclusion-and-recommendations)에 정리했다.
...
export LAW_OC=<본인_인증키>                       # 국가법령정보센터 OC (§2 참고)
...
- **위치 질의는 RAG가 아니다.** "○○법 제5조" 류 known-item 질의는 RAG 검색이 약하다(최고 recall 0.66). 법령명과 조번호를 파싱해 **직접 조회(MCP/CLI)**하는 구조적 경로가 정답이며, 이것이 MCP/CLI의 근거다([루트 README §3](../README.md#3-kfinlaw-with-mcp)).
