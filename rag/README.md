...
- **전체 recall이 낮다**(최고 0.66). 위치 질의는 RAG 검색보다 **구조적 조회**(법령명과 조번호를 파싱해 조문을 바로 가져오는 방식)가 정답이다.
...
- **위치 질의는 RAG가 아니다.** "○○법 제5조" 류 known-item 질의는 RAG 검색이 약하다(최고 recall 0.66). 법령명과 조번호를 파싱해 **직접 조회**하는 구조적 경로가 정답이다.
