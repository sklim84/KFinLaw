...
벤치마크는 `benchmark` 패키지다. 저장소 루트에서 `python -m benchmark.<모듈>`(데이터 수집은 `python scripts/<파일>.py`)로 실행하고, 모델은 전용 venv에서 `scripts/serve_model.sh {mistral|gptoss|stop}`로 띄운다. 버전 고정·서빙 플래그·디렉토리 구조는 아래 접은 글에 있다.
