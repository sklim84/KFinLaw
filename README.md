...
>
> - ✅ **검색은 '하이브리드 + 리랭커'가 정답.** 문서를 조·섹션 단위로 자르고, 키워드 검색(BM25)과 의미 검색(KURE 임베딩)을 함께 쓴 뒤 리랭커로 재정렬하면 정답 회수율(recall@5)이 0.86 수준으로 가장 높다(리랭커 4종은 동률). 특히 **리랭커가 성능을 가장 크게 끌어올리는 단일 요소**다. (그림 2)
...
> - ❌ **'정교한' 증강 기법은 오히려 손해.** 가설 질문(HyPE)·가설 답변(HyDE)·지식그래프(LightRAG) 세 가지 모두 기본 구성보다 낮았다. 정교하다고 더 좋은 게 아니므로 **새 데이터에 도입하기 전 반드시 직접 측정**해야 한다. (그림 6)
> - ✅ **답변 생성 LLM은 크기보다 도메인 적합성.** 같은 검색·프롬프트 조건에서 31B급 gemma-4가 100B(Solar)·67B(A.X)보다 더 나은 답을 냈다. 무작정 큰 모델보다 한국어·도메인에 맞는 중형을 실측 비교해 고르는 편이 낫다. (그림 9)
...
작업 순서는 벤치마크(목적1, [§2](#2-kfinlaw-with-rag)) → MCP([§3](#3-kfinlaw-with-mcp))·CLI([§4](#4-kfinlaw-with-cli)). 원칙은 **정석** — 공식 레시피·재현 가능 환경, 임시 우회 지양.
...
**엄밀성.** judge 독립성(답변모델과 다른 계열 — 같으면 자기우대 +10~25%), 레퍼런스 기반 채점(judge에 정답+정답조문 제공), 인간(법률가) 표본 검수 ~150문 + 일치계수(κ) 보고(ARES[[17]](#ref17)·KBL[[18]](#ref18) 관행)는 향후 과제.
...
청킹·임베딩·검색기·리랭커·증강을 바꿔가며 정보요구형 두 벤치마크(**Lexical · Semantic**)에서 회수 성능을 측정했다. 모든 검색 구성을 3개 메트릭(recall@5 · MRR · nDCG@10)으로 평가한 종합 순위는 아래 리더보드에 있다(평균 recall@5 순). 청킹 조(條) 단위·임베딩 KURE-v1 고정.
...
> **하이브리드+리랭커가 두 벤치마크·전 메트릭에서 고르게 상위.** 증강(HyDE·HyPE·LightRAG)은 모두 자기 baseline 아래다(상세는 증강 그리드, 표 3). 리랭커 4종 중 ko-reranker-8k는 Lexical 최고지만 Semantic 급락 — 평균이 가장 균형 잡힌 **bge-reranker-v2-m3가 기본값(🏆)** 이다.

<img src="benchmark/figures/fig_01_leaderboard.png" width="700" alt="검색 구성 리더보드">

> **그림 2.** 검색 구성별 Lexical·Semantic recall@5(막대 번호는 위 표의 #, 위가 평균 상위).

**실험별 요점.**

> **표 2.** 실험별 결과 요약 — 비교 축별 결론과 근거 (Lexical).
...
> **그림 3.** 청킹 단위별 recall@5. &nbsp; **그림 4.** 검색기 × 질문유형별 recall@5.
> **그림 5.** 컴포넌트 누적 recall@k. &nbsp; **그림 6.** 증강 기법 vs baseline.
...
§2.2의 Lexical Benchmark는 조문에서 질문을 만들고 BM25 일관성 필터로 거르므로, 질문이 조문 용어를 그대로 쓰는(어휘중첩이 큰) 쪽으로 치우친다. 실제로 구어체 질문조차 격식체와 비슷하거나 더 쉽게 검색됐는데(벡터 KURE 격식 0.733 vs 구어 1.000, 그림 7), 질의–조문 어휘격차가 애초에 작았다는 신호다. 그렇다면 BM25 우위나 HyPE·HyDE 무용이 이 편향의 산물일 수 있다 — 이를 가리려 일상어로 풀어 어휘격차를 키운 **Semantic Benchmark**를 만들었다(생성·필터는 §2.2).
...
> **그림 9.** 답변모델 5종 × 6개 메트릭 judge 점수 히트맵(진할수록 높음).
