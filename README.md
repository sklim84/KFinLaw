...
| 비교 축 | 결론 | 근거 |
|---|---|---|
| 청킹 | **조(條) 단위 최적** | BM25·벡터 양쪽 최고 (고정 토큰은 벡터에 불리, 계층(부모 문서)은 벡터 랭킹↑) |
...
| 증강 (HyDE·HyPE) | **효과 없음** ❌ | 전 기저에서 baseline 이하(표 3), 둘을 겹치면 최악 |
...
증강(HyDE·HyPE·LightRAG)은 어느 기저에서도 baseline을 넘지 못했다. 그림 5처럼 어느 칸도 기저를 넘지 못하고, HyDE+HyPE 중첩이 가장 나쁘다.

<details>
<summary>세부 수치, 증강 그리드 · 검색기 × 질문유형 · LightRAG 모드별</summary>

> **표 3.** 증강 그리드, HyDE·HyPE를 기저별로 적용한 recall@5 (Lexical). 어느 칸도 기저(none)를 넘지 못한다.
...
리랭커가 손상을 크게 복구하지만(예: 벡터+HyDE에 리랭커 추가 시 0.652→0.758) 여전히 기저 미만이고, **HyDE+HyPE 중첩이 가장 나쁘다**(0.562). 하이브리드에 HyPE를 적용해도(BM25는 원문, dense는 가설질문 임베딩으로 RRF 결합) 0.831→0.762로 떨어진다. 다만 리랭커를 더하면 하이브리드+HyPE가 기저값(0.860)까지 복구되는데, 그때도 baseline을 넘지는 못한다.
...
- **BM25는 붕괴(−39%)하고 벡터는 그대로라 순위가 뒤집힌다.** 어휘격차가 큰 현실적 질의에선 벡터가 BM25를 크게 앞선다. Lexical Benchmark의 BM25 우위는 상당 부분 어휘중첩 인공물이었다.
...
- **리랭커가 결정적.** 크로스인코더가 브레드크럼("[법령명 > 제N조]")을 통째로 읽어 정확 매칭하므로, 동일 검색기(하이브리드)에서 0.443→0.660, 단일 조는 0.85까지 오른다. (BM25 0.197 대비 최적 구성 전체로는 +0.46.)
