...
[한눈에](#한눈에-핵심-결론) · [개요](#개요) · [핵심 결과](#핵심-결과) · [빠른 시작](#빠른-시작) · [벤치마크 설계](#벤치마크-설계) · [모델·서빙](#모델서빙) · [답변 평가](#답변-평가) · [데이터·구조](#데이터구조) · [References](#references)

---

## 한눈에 (핵심 결론)

한국 금융 법령 240문 벤치마크에서 얻은 핵심 결론:
...
검색 실험은 변수를 하나씩만 바꿔 비교한다(E1 청킹 · E2 임베딩 · E3 검색기+리랭커 · E4 별표소스 · E5 HyPE · E6 그래프 · E7 HyDE).
셋업: 코퍼스 32법령(~3,251 청크), 골드셋 240문(유형 4종 각 60), gold = 조문/별표 uid. 본문 인용은 (저자 연도) — 출처는 하단 [References](#references).
...
> **그림 2.** 청킹 단위(E1) — 조(條) 단위가 BM25·벡터 양쪽 최고. &nbsp; **그림 3.** 검색기 × 질문유형 — factoid는 dense, crossref·별표는 BM25가 강함.
> **그림 4.** 컴포넌트 누적 recall@k — 리랭커가 단일 최대 레버. &nbsp; **그림 5.** 증강 기법(HyPE·HyDE·LightRAG) 모두 baseline 미달 — "정교한 기법 ≠ 더 나음".

<details>
<summary>검색기 × 질문유형 · 반직관적 발견 · 적용 권고</summary>
...
> **그림 0.** 방법론 개요 — 데이터(코퍼스·골드셋) → 레이어1 검색 평가 → (최적 config 고정) → 레이어2 답변 평가. 변수는 한 번에 하나씩만 바꿔(OFAT) 기법별 효과를 격리한다.
...
> **시사점**: 답변모델은 무작정 큰 모델이 아니라 한국어·도메인 적합 중형(예: gemma-4-31B급)을 동일 검색·프롬프트에서 실측 비교해 선정하는 것이 유효.
