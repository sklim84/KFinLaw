...
| 증강(HyDE·HyPE) | **효과 없음** ❌ | 모든 기저에서 적용 전보다 낮음 (표 3) |
| LightRAG | **효과 없음** ❌ | 그래프 모드가 naive보다 낮음 (표 1) |
...
> **표 3.** 증강 그리드, HyDE·HyPE를 기저별로 적용한 recall@5 (Lexical). 어느 조합도 기저(none)를 넘지 못한다.
...
리랭커가 하락폭을 크게 줄이지만(예: 벡터+HyDE에 리랭커를 더하면 0.652→0.758) 여전히 기저에 못 미치고, **HyDE+HyPE 중첩이 가장 나쁘다**(0.562). 하이브리드에 HyPE를 적용해도(BM25는 원문, dense는 가설질문 임베딩으로 RRF 결합) 0.831→0.762로 떨어진다. 다만 리랭커를 더하면 하이브리드+HyPE가 기저값(0.860)까지 복구되지만, 그래도 baseline을 넘지는 못한다.

<details>
<summary>세부 표: 검색기×질문유형 · LightRAG 모드별</summary>
...
Semantic에서 같은 기법들을 다시 평가하니 검색기 간 우열이 뒤집힌다(그림 6·표 4).
