...
**답변 메트릭**은 인용 F1만 자동·객관이고, 나머지 다섯 항목은 심사모델 루브릭으로 채점한다. 심사모델(gpt-oss-120b[[28]](#ref28))은 답변모델과 다른 계열을 쓰고, 검색은 최적 구성(하이브리드+리랭커)으로 고정해 답변모델 효과만 본다. 생성·심사·답변 LLM을 서로 다른 계열로 나눈 것은, 같은 계열이면 심사모델이 자기 쪽을 후하게 평가하는 편향이 생기기 때문이다. 모든 모델은 temp=0으로 실행한다.
