...
**청킹·임베딩·검색기 중 무엇이 검색 성능을 좌우하고, 인기 증강기법은 실제로 도움이 되는가?** 변수를 하나씩 격리해(OFAT) 각 축의 기여를 분리했다. 축별 결론·근거는 아래 표에, 청킹·검색기·증강의 분해는 이어지는 그림에 있다.
...
**위치 질의를 RAG 검색으로 풀 수 있는가?** "○○법 제5조", "제5조부터 제8조까지"처럼 사용자가 **위치(법령명과 조번호)를 이미 알고** 그 조문을 찾는 known-item 질의다. 정보요구형(Lexical·Semantic)과 달리 정답이 위치로 정해지며, 벡터 검색이 정확한 식별자를 얼마나 잘 다루는지 본다.
...
한국 금융 법령을 테스트베드로 RAG 검색·답변 기법을 통제 실험으로 비교한 결과와 실무 권고는 다음과 같다.

- **권장 기본 구성.** 조(條) 단위 청킹 + KURE-v1 임베딩 + **하이브리드(BM25+KURE) + 리랭커(bge-reranker-v2-m3)**. 두 벤치마크·전 메트릭에서 고르게 상위이며, 리랭커 *적용*이 단일 최대 레버다.
- **도입하지 말 것.** HyPE·HyDE·LightRAG 등 증강기법은 어느 기저에서도 baseline에 미달했다. 정교함이 곧 성능이 아니므로, 새 도메인에 들이기 전 반드시 직접 측정한다.
