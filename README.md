...
[**한눈에**](#한눈에) · [1. 개요](#1-개요) · [2. 벤치마크 설계](#2-벤치마크-설계) · [3. 결과](#3-결과) · [4. 어휘중첩 편향 검증](#4-어휘중첩-편향-검증-lexical-vs-semantic-benchmark) · [5. Locator](#5-locator-benchmark--조문-위치-질의) · [6. 실험 환경](#6-실험-환경) · [7. 데이터 파이프라인](#7-데이터-파이프라인) · [References](#references)
...
> - ✅ **답변 생성 LLM은 크기보다 도메인 적합성.** 같은 검색·프롬프트 조건에서 31B급 gemma-4가 100B(Solar)·67B(A.X)보다 더 나은 답을 냈다. 무작정 큰 모델보다 한국어·도메인에 맞는 중형을 실측 비교해 고르는 편이 낫다. (그림 6)
...
> - 📍 **위치 지정 질의는 RAG의 약점.** "○○법 제5조"처럼 조 위치를 직접 묻는 known-item 질의(Locator)에선 BM25·dense 모두 약하고(최고 recall 0.66) 리랭커가 결정적이다 — RAG보다 **구조적 조회(목적2)**가 맞는 영역. (§5)
...
이 240문은 조문 용어가 자연스럽게 섞여 어휘중첩이 큰 **Lexical Benchmark**다. 이와 짝으로, 같은 코퍼스·유형 분포에서 전문용어를 일상어로 풀어 어휘격차를 크게 만든 **Semantic Benchmark**(240문)를 추가로 두어, 검색기들이 표면 어휘중첩에 얼마나 의존하는지 검증한다(생성·필터 방식과 결과는 [§4](#4-어휘중첩-편향-검증-lexical-vs-semantic-benchmark)). 두 벤치마크는 모두 동일한 골드셋 스키마를 따른다. 한편 사용자가 조 위치를 직접 지정하는 known-item 질의("○○법 제5조")는 성격이 달라 별도의 **Locator Benchmark**(103문, 결정론 생성)로 다룬다([§5](#5-locator-benchmark--조문-위치-질의)).
...
파이프라인 단계별로 아래 선택지를 바꿔가며 비교했다. 검색 결과는 질문유형(factoid·crossref·byeolpyo·multihop)과 register(격식체/구어체)별로도 분해해 기법의 강·약점을 살핀다.
...
| 재순위 | 적용 여부 + 리랭커 4종 비교 (bge-reranker-v2-m3[[5]](#ref5) · 한국어 · 영어 대형) |
...
청킹·임베딩·검색기·리랭커·증강 기법을 바꿔가며 **Lexical·Semantic 두 벤치마크(각 240문)** 에서 회수 성능을 측정했다. 종합 순위는 아래 리더보드에, 실험별 요점은 이어지는 표와 그림에 정리한다(Lexical→Semantic 어휘중첩 편향 분석은 [§4](#4-어휘중첩-편향-검증-lexical-vs-semantic-benchmark), 위치 지정 질의는 [§5](#5-locator-benchmark--조문-위치-질의)).

#### 3.1.1 종합 리더보드

정보요구형 두 벤치마크(**Lexical · Semantic**)에서 모든 검색 구성을 **3개 메트릭(recall@5 · MRR · nDCG@10)** 으로 평가했다(평균 recall@5 순). 청킹 조(條) 단위·임베딩 KURE-v1 고정, 리랭커 4종 비교(bge-m3=bge-reranker-v2-m3 · ko=ko-reranker · ko-8k=ko-reranker-8k · large=bge-reranker-large). 증강 전체 그리드는 [§3.1.2](#312-실험별-결과), 위치 지정(Locator) 질의는 성격이 달라 [§5](#5-locator-benchmark--조문-위치-질의)에서 별도로 다룬다.

| # | 검색기 | 리랭커 | 증강 | **Lexical**<br>R@5 | MRR | nDCG | **Semantic**<br>R@5 | MRR | nDCG | 평균<br>R@5 |
|:-:|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| 1 🏆 | 하이브리드(BM25+KURE) | bge-m3 | — | 0.860 | 0.775 | 0.793 | 0.756 | 0.661 | 0.684 | **0.808** |
| 2 | 하이브리드(BM25+KURE) | ko | — | 0.838 | 0.729 | 0.754 | 0.769 | 0.672 | 0.691 | **0.803** |
| 3 | 벡터(KURE) | bge-m3 | — | 0.808 | 0.748 | 0.751 | 0.777 | 0.661 | 0.684 | **0.793** |
| 4 | 하이브리드(BM25+KURE) | ko-8k | — | 0.863 | 0.785 | 0.804 | 0.708 | 0.608 | 0.637 | **0.785** |
| 5 | 하이브리드(BM25+KURE) | bge-m3 | HyDE | 0.812 | 0.743 | 0.750 | 0.742 | 0.657 | 0.669 | **0.777** |
| 6 | 벡터(KURE) | — | — | 0.767 | 0.656 | 0.672 | 0.767 | 0.612 | 0.648 | **0.767** |
| 7 | 하이브리드(BM25+KURE) | large | — | 0.798 | 0.699 | 0.719 | 0.729 | 0.621 | 0.644 | **0.764** |
| 8 | LightRAG(naive) | — | — | 0.738 | 0.639 | 0.648 | 0.767 | 0.616 | 0.649 | **0.752** |
| 9 | 하이브리드(BM25+KURE) | — | — | 0.831 | 0.710 | 0.735 | 0.658 | 0.562 | 0.588 | **0.745** |
| 10 | BM25 | — | — | 0.835 | 0.707 | 0.741 | 0.510 | 0.397 | 0.430 | **0.673** |
| 11 | LightRAG(mix) | — | — | 0.665 | 0.606 | 0.616 | 0.665 | 0.552 | 0.572 | **0.665** |
| 12 | 벡터(KURE) | — | HyPE | 0.721 | 0.622 | 0.637 | 0.579 | 0.500 | 0.521 | **0.650** |
| 13 | 벡터(KURE) | — | HyDE | 0.652 | 0.558 | 0.572 | 0.592 | 0.469 | 0.499 | **0.622** |

> **하이브리드+리랭커가 두 벤치마크·전 메트릭에서 고르게 상위.** 증강(HyDE·HyPE·LightRAG)은 모두 자기 baseline 아래다(상세 그리드 [§3.1.2](#312-실험별-결과)). 리랭커 4종 중 ko-reranker-8k는 Lexical 최고지만 Semantic 급락 — 평균이 가장 균형 잡힌 **bge-reranker-v2-m3가 기본값(🏆)** 이다.

<img src="benchmark/figures/fig_01_leaderboard.png" width="700" alt="검색 구성 리더보드">

> **그림 1.** 검색 구성별 Lexical·Semantic recall@5(위가 평균 상위). 하이브리드+리랭커가 양쪽 모두 상위, 증강·BM25는 하위.
...
| LightRAG | **효과 없음** ❌ | 그래프 모드가 naive(순수 벡터 검색)보다 낮아 그래프화가 역효과, 평균 순위도 단일 벡터 아래([§3.1.1](#311-종합-리더보드)) |
...
**증강 그리드** — HyDE·HyPE를 적용 가능한 모든 dense 기저에 적용한 recall@5. **모든 칸이 기저(none)보다 낮다.**
...
검색을 하이브리드+리랭커로 고정한 Lexical Benchmark 240문 기준이며, 점수는 0~1이다. Semantic Benchmark 재평가는 [§4.2](#42-답변-재평가)에서 다룬다.
...
> **그림 6.** 답변모델 5종 × 6개 메트릭 히트맵. gemma-4-31B가 전 항목 최상위, A.X-4.0이 근접.

<details>
<summary>RAGAS 대응 · 고유 비교 축 · 엄밀성</summary>

#### RAGAS 대응 (정렬했으나 라이브러리 비채택)
RAGAS[[16]](#ref16)는 RAG 평가의 사실상 표준이다(reference-free LLM 자동채점). 우리 메트릭도 그 분류에 의도적으로 맞췄다. 다만 라이브러리는 쓰지 않고 직접 구현했는데, ① gold(정답·정답조문 uid)로 레퍼런스 기반의 더 엄밀한 채점, ② judge 계열 강제 분리, ③ 조문 uid 인용검증·한국어 프롬프트 직접 통제를 위해서다.
...
**검증 — Semantic Benchmark.** 이 의심이 맞는지 확인하려고 두 번째 벤치마크를 만들었다. 같은 코퍼스·유형 분포에서 법령명·조문번호·고유 전문용어를 빼고 일상어로 풀어 묻게 생성하고, **검색기와 무관한 결정론적 필터**로 거른다 — 코퍼스 문자 4-gram의 문서빈도(DF)로 '희소 용어'를 판정해, 질문이 정답 조문의 희소 4-gram을 재사용하면 폐기한다(생성 시도의 31%가 누출로 폐기). BM25 일관성 필터를 쓰지 않아 어휘중첩 편향이 재유입되지 않는다.
...
아래 표는 **Semantic recall@5 기준으로 정렬**해 그대로 Semantic 리더보드 역할을 한다(Lexical 열·Δ로 변화를 함께 본다).
...
- **임베더 강건성 차이.** KoE5가 어휘격차에 가장 강건(임베더 중 유일하게 상승, +0.046), KURE는 최고 성능을 그대로 유지(±0). (KURE>KoE5>BGE-M3 전체 비교는 [§3.1.2](#312-실험별-결과))
- **증강 기법의 부정 결과는 두 벤치마크 모두에서 견고하다.** HyPE·HyDE·LightRAG는 Semantic에서도 단순 벡터에 못 미친다. 즉 "정교한 증강이 효과 없다"는 [§3.1.2](#312-실험별-결과)의 결론은 골드셋 편향 탓이 아니라 견고한 결과다.
...
§3.2의 답변 평가를 Semantic Benchmark에서 그대로 반복했다(검색은 그 최적인 벡터+리랭커로 고정, judge 점수 6개 항목 평균). **순위는 그대로 — gemma-4-31B 1위·A.X-4.0 2위** 로, 답변모델 선택은 질의 표현에 견고하다. 검색 recall이 낮아진 만큼(0.860→0.777) 답변 품질은 **소폭만 하락**(대부분 −0.03~0.06)해 파이프라인이 어휘격차에서도 끝까지 견딘다. 한편 Solar-100B가 가장 크게 하락(−0.056)해 "[크기보다 도메인 적합성](#한눈에)" 결론은 더 강화됐다.
...
## 5. Locator Benchmark — 조문 위치 질의

"○○법 제5조", "제5조부터 제8조까지"처럼 사용자가 **위치(법령명+조번호)를 이미 알고** 그 조문을 찾는 known-item 질의다. 정보요구형(Lexical·Semantic)과 달리 정답이 위치로 특정되며, dense 검색의 '정확 식별자' 처리력을 본다. 코퍼스에서 **결정론적으로 생성**(LLM 불요)한 103문 — L1 단일 조(40) · L2 조 범위(23, 멀티-gold) · L3 법령명 생략·모호(40).
...
- **전체 회수율이 낮다**(최고 0.66). locator는 RAG 검색보다 **구조적 조회**(법령명+조번호 파싱 → 직접 fetch)가 정답이며, 이것이 [목적2 MCP/CLI](#1-개요)의 직접적 근거다.
