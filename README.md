...
파이프라인 단계별 선택지를 한 번에 하나씩만 바꾸며(일변수 격리) 비교했다. 아래 표에 단계별 비교 선택지를 정리한다(답변모델 5종은 답변 평가(2단계) 비교 대상, 골드셋 생성기·심사기는 고정).

| 단계 | 선택지 | 소개 |
|---|---|---|
| **파싱**<br>(별표 소스) | kordoc-md | HWP·PDF 별표를 GPU 없이 마크다운 표로 변환(라이선스 자유), 기본 변환기 |
| | 평문 | 별표를 표 구조 없이 평문 텍스트로, 표구조 보존 효과의 대조 기준 |
| | MinerU(미연결) | 병합셀이 복잡한 표에 더 정확한 레이아웃 파서지만, 파이프라인 미연결·비교만 수행 |
| **청킹** | 조(article) | 법령 조문 단위로 분할, 법령의 자연 단위(**기본값**) |
...
| **재순위**<br>(리랭커) | bge-reranker-v2-m3[[5]](#ref5) | BGE-M3 기반 다국어 크로스인코더, **권장 기본값**(균형 최상) |
| | ko-reranker | 한국어 특화 크로스인코더 |
| | ko-reranker-8k | 한국어·긴 컨텍스트(8k) 크로스인코더 |
| | bge-reranker-large | 영어 대형 크로스인코더, 규모 대조군 |
| **증강** | HyPE(색인측)[[10]](#ref10) | 색인 시 각 청크의 가설 질문을 생성·임베딩해 질문↔질문으로 매칭 |
...
| 생성기<br>(고정) | Mistral Small 4 (119B) | 골드셋·HyPE/HyDE 질의 생성. 답변모델과 독립 계열, 한국어·별표 우수 |
| 심사기<br>(고정) | gpt-oss-120b | 답변 채점(judge). 생성기·답변모델과 다른 계열로 오염 방지 |

> *임베딩* 3종은 모두 **1024차원**이고 XLM-RoBERTa-large 백본을 공유한다(컨텍스트는 KURE·BGE-M3 8192, KoE5 512). 차원이 같아 임베더 비교가 *차원 통제* 상태에서 이뤄진다. *재순위*는 리랭커 *적용 여부 자체*가 종류 선택보다 큰 성능 레버다. *LLM 역할*(생성기·심사기·답변모델)은 서로 다른 계열로 분리하며(같은 계열이면 self-enhancement·preference leakage 발생), 모든 모델은 temp=0으로 실행한다.
