...
핵심 검색 구성을 **세 벤치마크(Lexical·Semantic·Locator) recall@5**로 평가했다(평균순). 청킹은 조(條) 단위, 임베딩은 KURE-v1 고정. 리랭커는 4종 비교. 증강(HyDE·HyPE·LightRAG)은 전부 baseline 미달이라 [§3.1.2](#312-실험별-결과)에 따로 둔다.

| 구성 | Lexical | Semantic | Locator | 평균 |
|---|:-:|:-:|:-:|:-:|
| 하이브리드 + 리랭커(bge-reranker-v2-m3) 🏆 | 0.860 | 0.756 | 0.660 | **0.759** |
| 하이브리드 + 리랭커(ko-reranker) | 0.838 | 0.769 | 0.669 | 0.758 |
| 하이브리드 + 리랭커(ko-reranker-8k) | **0.863** | 0.708 | **0.677** | 0.749 |
| 벡터(KURE) + 리랭커 | 0.808 | **0.777** | 0.626 | 0.737 |
| 하이브리드 + 리랭커(bge-reranker-large) | 0.798 | 0.729 | 0.666 | 0.731 |
| 하이브리드(BM25+KURE) | 0.831 | 0.658 | 0.443 | 0.644 |
| 벡터(KURE) | 0.767 | 0.767 | 0.376 | 0.636 |
| BM25 | 0.835 | 0.510 | 0.197 | 0.514 |

> **하이브리드+리랭커가 세 벤치마크 모두 고르게 강하다.** BM25는 Lexical만 높고 Semantic·Locator에서 무너지며, 리랭커 4종 중 ko-reranker-8k는 Lexical 최고지만 Semantic에서 급락 — 평균이 가장 균형 잡힌 **bge-reranker-v2-m3를 기본값(🏆)** 으로 쓴다.
