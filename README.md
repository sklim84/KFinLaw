...
> 벤치마크는 `benchmark` 패키지다 — **repo 루트에서 `python -m benchmark.<모듈>`** 으로 실행한다
> (sys.path 조작 없음). 데이터 수집기는 독립 스크립트라 `python scripts/<파일>.py`.
...
# 벤치마크
python -m benchmark.corpus                    # 코퍼스 32법령
#  (vLLM 기동) scripts/serve_model.sh mistral
python -m benchmark.goldset.build_goldset --model mistralai/Mistral-Small-4-119B-2603 \
