...
| **임베딩**<br>(셋 다 **1024-d**,<br>XLM-R-large 백본) | KURE-v1[[8]](#ref8) | 한국어 검색 특화 임베더, BGE-M3를 한국어로 파인튜닝, **기본 임베더**(ctx 8192) |
| | BGE-M3[[7]](#ref7) | 다국어·다기능·다입도 임베딩, KURE의 베이스 모델, 다국어 일반 성능 기준(ctx 8192) |
| | KoE5[[6]](#ref6) | multilingual-e5-large를 한국어로 파인튜닝, E5 계열 대조(ctx 512) |
...
| **재순위**<br>(리랭커;<br>적용 여부가<br>최대 레버) | bge-reranker-v2-m3[[5]](#ref5) | BGE-M3 기반 다국어 크로스인코더, **권장 기본값**(균형 최상) |
...
| **답변 생성** | 답변모델 5종 + 변형 | 컨텍스트 포맷·인용 지시를 변형(모델은 아래 역할 표) |
