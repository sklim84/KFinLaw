...
- **Lexical Benchmark** (240문항): 생성모델 Mistral Small 4 (119B)[[27]](#ref27)가 각 조문·별표에서 Q&A를 생성하고, **일관성 필터**로 거른다. 이 필터는 생성된 질문을 다시 검색기에 넣어 원래 조문이 검색되는 질문만 채택한다(round-trip BM25). 정의·요건(factoid), 교차참조(crossref), 별표 조회(byeolpyo), 본법·시행령 멀티홉(multihop) 네 유형을 각 60문항씩 균형 있게 구성했다. 멀티홉은 본법과 시행령을 함께 짚어 정답이 2개씩이라 정답 식별자는 모두 300개다(전부 코퍼스에 존재함을 확인했다). factoid에는 같은 사실을 격식체와 구어체로 묻는 30쌍을 넣어 어휘격차를 짝지어 본다. 질문이 조문 용어를 자연스럽게 써서 어휘중첩이 큰 것이 특징이다.
- **Semantic Benchmark** (240문항): Lexical과 같은 코퍼스·유형 분포를 쓰되, 전문용어를 일상어로 풀어 어휘격차를 크게 키웠다. 검색기가 표면 어휘중첩에 얼마나 기대는지 가린다.
- **Locator Benchmark** (103문항): 사용자가 조 위치를 직접 지정하는 known-item 질의("○○법 제5조")로, LLM 없이 코퍼스에서 결정론적으로 생성한다. 정보요구형(Lexical·Semantic)과 성격이 달라 따로 둔다.

<details>
<summary>골드셋 방법론 근거</summary>

검색 골드셋은 정답이 ID라 LLM-judge 편향과 무관하므로 오픈웨이트 생성모델로 만들어도 충분하다(InPars[[12]](#ref12); InPars-v2[[13]](#ref13)). 다만 생성모델·judge·답변모델은 서로 다른 계열로 분리해 선호 누출(preference leakage)을 피했고, 품질의 핵심 장치는 일관성 필터(Promptagator[[14]](#ref14))다. 생성된 질문을 검색기에 다시 넣어 원본 조문이 다시 검색되는 질문만 채택한다. 한 가지 주의로, BM25가 구어체를 과소평가할 수 있어 factoid 쌍은 격식체로 일관성을 검사하되 같은 정답을 공유하는 구어체도 함께 채택했다.
...
**검색 메트릭**(`benchmark/eval/retrieval_metrics`)은 검색된 청크의 uid를 gold uid와 대조해 recall@k · precision@k · MRR · nDCG@k[[15]](#ref15)를 **결정론적으로** 계산하므로 LLM 판정이 개입하지 않는다.
...
**엄밀성.** judge 독립성(답변모델과 다른 계열, 같으면 자기우대 +10~25%), 레퍼런스 기반 채점(judge에 정답+정답조문 제공), 인간(법률가) 표본 검수 ~150문항 + 일치계수(κ) 보고(ARES[[17]](#ref17)·KBL[[18]](#ref18) 관행)는 향후 과제다.
...
**(RQ5) 위치 질의를 RAG 검색으로 풀 수 있는가?** "○○법 제5조", "제5조부터 제8조까지"처럼 사용자가 **위치(법령명과 조번호)를 이미 알고** 그 조문을 찾는 known-item 질의다. 정보요구형(Lexical·Semantic)과 달리 정답이 위치로 특정되며, 벡터 검색의 '정확 식별자' 처리력을 본다.
...
**(RQ4) 답변 품질은 모델 크기가 좌우하는가, 도메인 적합성이 좌우하는가?** 검색을 최적 구성(하이브리드+리랭커)으로 고정하고 답변모델만 바꿔, 검색이 찾은 근거 위에서 만든 답의 품질을 본다(Lexical Benchmark 240문항, judge 점수 0~1).
