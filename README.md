...
청킹·임베딩·검색기·리랭커·증강을 바꿔가며 정보요구형 두 벤치마크(**Lexical · Semantic**)에서 회수 성능을 측정했다. 청킹은 조(條) 단위로, 임베딩은 KURE-v1으로 고정하고 나머지를 변화시켰으며, 모든 검색 구성을 3개 메트릭(recall@5 · MRR · nDCG@10)으로 평가했다. 그 종합 순위를 평균 recall@5 순으로 아래 리더보드에 정리했다.
