...
[**한눈에**](#한눈에) · [1. 개요](#1-개요) · [2. 벤치마크 설계](#2-벤치마크-설계) · [3. 결과](#3-결과) · [4. 참고사항](#4-참고사항) · [5. 재현 및 환경](#5-재현-및-환경) · [6. 데이터·구조](#6-데이터구조) · [References](#references)
...
> - ✅ **답변 생성 LLM은 크기보다 도메인 적합성.** 같은 검색·프롬프트 조건에서 31B급 gemma-4가 96B(Solar)·67B(A.X)보다 더 나은 답을 냈다. 무작정 큰 모델보다 한국어·도메인에 맞는 중형을 실측 비교해 고르는 편이 낫다. (그림 7)
...
**리더보드** — recall@5 / MRR / nDCG@10 (240문)
...
**반직관적 발견 — 구어체 ≥ 격식체.** 구어체 질문이 격식체보다 검색이 더 쉬웠다(벡터 KURE 격식 0.733 vs 구어 1.000, 그림 6). 메울 질의–조문 어휘격차가 애초에 작다는 뜻이며, 이것이 가설질문·가설답변(HyPE·HyDE) 증강이 효과 없던 이유를 설명한다 — "정교한 기법 ≠ 더 나음, 도입 전 실측". 적용 시 권고는 [한눈에](#한눈에) 요약을 참고.

<img src="benchmark/figures/fig_06_register.png" width="460" alt="격식체 vs 구어체">
