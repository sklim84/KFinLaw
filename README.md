...
위 코퍼스에서 평가용 **240문**을 반자동으로 만들었다. 생성모델 Mistral Small 4 (119B)[[27]](#ref27)가 각 조문·별표에서 Q&A를 생성하고, **일관성 필터**로 거른다. 이 필터는 생성된 질문을 다시 검색기에 넣어 원래 조문이 회수되는 질문만 채택한다(round-trip BM25).

질문은 네 유형을 각 60문씩 균형 있게 두었다: factoid(정의·요건), crossref(교차참조), byeolpyo(별표 조회), multihop(본법↔시행령). 정답 근거(gold)는 조문·별표 식별자로 고정했고, multihop 60문은 본법과 시행령 둘을 함께 짚는 2-gold다. 그래서 정답 식별자는 1-gold 180문에서 180개, 2-gold 60문에서 120개로 모두 300개다. factoid에는 같은 사실을 격식체·구어체로 묻는 30쌍을 넣어 어휘격차를 짝지어 측정한다. gold uid 300개가 모두 코퍼스에 존재함을 확인했다.
...
**(RQ1·RQ2) 청킹·임베딩·검색기 중 무엇이 회수를 좌우하고, 인기 증강기법은 실제로 도움이 되는가?** 변수를 하나씩 격리해(OFAT) 각 축의 기여를 분리했다. 축별 결론·근거는 아래 표에, 청킹·검색기·증강의 분해는 이어지는 그림에 있다.
...
- **전체 회수율이 낮다**(최고 0.66). locator는 RAG 검색보다 **구조적 조회**(법령명과 조번호를 파싱해 조문을 바로 가져오는 방식)가 정답이며, 이것이 [목적2 MCP/CLI](#1-introduction)의 직접적 근거다.
