...
| LightRAG | **효과 없음** ❌ | 그래프 모드가 단순 벡터 검색보다 낮음 (표 1) |
...
리랭커가 하락폭을 크게 줄이지만(예: 벡터+HyDE에 리랭커를 더하면 0.652→0.758) 여전히 증강 전보다 낮고, **HyDE+HyPE 중첩이 가장 나쁘다**(0.562). 하이브리드에 HyPE를 적용해도(BM25는 원문, dense는 가설질문 임베딩으로 RRF 결합) 0.831→0.762로 떨어진다. 다만 리랭커를 더하면 하이브리드+HyPE가 증강 전 값(0.860)까지 회복되지만, 그 이상으로 올라가지는 못한다.
