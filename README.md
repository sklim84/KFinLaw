...
[**한눈에**](#한눈에) · [1. 개요](#1-개요) · [2. 벤치마크 설계](#2-벤치마크-설계) · [3. 결과](#3-결과) · [4. 논의 및 적용 권고](#4-논의-및-적용-권고) · [5. 재현 및 환경](#5-재현-및-환경) · [6. 데이터·구조](#6-데이터구조) · [References](#references)
...
> [!TIP]
> **핵심 결론** — 한국 금융 법령 240문 벤치마크 요약
>
> - ✅ **검색 기본기**: 조/섹션 단위 청킹 + **하이브리드(BM25+KURE) + 리랭커** → recall@5 **0.860** (그림 1). 리랭커가 단일 최대 레버.
> - ✅ **한국어 임베딩**: **KURE-v1** (> KoE5 > BGE-M3). dense는 의미질문, BM25는 명칭·번호·표 어휘에 강함 → 둘을 합친 하이브리드.
> - ❌ **굳이 안 해도 되는 것**: 정교한 증강 3종 모두 baseline 미달 — **HyPE·HyDE(가설 질문/답변)·LightRAG(지식그래프)** (그림 5). "정교한 기법 ≠ 더 나음 → 도입 전 반드시 실측."
> - ✅ **답변모델**: 크기보다 도메인 적합성 — **gemma-4-31B가 Solar-100B·A.X-67B를 능가** (그림 7). 동일 검색·프롬프트에서 실측 비교해 선정.
> - 🧭 **방법론**: 골드셋(LLM 생성 + 일관성 필터) 위에서 **검색→답변 2계층**으로, 변수를 한 번에 하나씩만 바꿔 평가(그림 0).
...
검색(레이어1)을 먼저 최적화·고정한 뒤 답변 생성(레이어2)을 평가하는 **2계층 파이프라인**. 변수는 한 번에 하나씩만 바꿔(OFAT) 기법별 효과를 격리한다.

![방법론 파이프라인](benchmark/figures/fig_00_pipeline.png)
> **그림 0.** 방법론 개요 — 데이터(코퍼스·골드셋) → 레이어1 검색 평가 → (최적 config 고정) → 레이어2 답변 평가.

**코퍼스** — 핵심 금융법 **32개 법령(13개 법령군)**. 각 군이 본법 + 시행령(+ 시행규칙)을 갖춰 법↔시행령 멀티홉·별표 조회·교차참조를 모두 평가하도록 통제된 소규모 구성.
...
7개 실험으로 변수를 하나씩 비교 — E1 청킹 · E2 임베딩 · E3 검색기+리랭커 · E4 별표소스 · E5 HyPE · E6 그래프 · E7 HyDE.
...
> **그림 2.** 청킹 단위(E1) — 조(條) 단위가 양쪽 최고. &nbsp; **그림 3.** 검색기 × 질문유형 — factoid는 dense, crossref·별표는 BM25.
> **그림 4.** 컴포넌트 누적 recall@k — 리랭커가 단일 최대 레버. &nbsp; **그림 5.** 증강 기법 모두 baseline 미달.
...
검색을 **최적 config(하이브리드+리랭커)로 고정**하고 답변모델만 바꿔 "답이 정확·충실한가"를 평가. judge는 답변모델과 다른 계열(gpt-oss-120b, temp=0), 인용정확도만 자동·객관. 검색 config는 `config.yaml › answer_eval`에 고정, `--retrieval {good,bad,none}`로 검색품질 전이·closed-book 대조도 가능.
...
export LAW_OC=<본인_인증키>                       # 국가법령정보센터 OC (§데이터·구조에서 발급)
...
**모델 역할 & 서빙** — 3개 역할을 서로 다른 계열로 분리(생성·judge가 답변모델과 같으면 self-enhancement·preference leakage). 전 구간 temp=0.

| 역할 | 모델 | 비고 |
|---|---|---|
| 골드셋 생성기 | Mistral Small 4 (119B) | 독립 계열, 한국어 우수(별표도 한국어 유지) |
| judge | gpt-oss-120b | 생성기·답변모델과 다른 계열 |
| 답변모델 (답변 평가) | A.X-4.0 · Solar-Open · EXAONE-4.0 · Gemma 4 · Qwen3.6 | 평가 대상 후보 |
...
│   ├── hype_index.py · hyde_gen.py   # HyPE(E5) · HyDE(E7) 증강
