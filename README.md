...
먼저 검색 평가를 최적화해 그 구성을 고정한 뒤, 그 위에서 답변 생성 평가를 수행하는 **2단계 파이프라인**이다. 변수는 한 번에 하나씩만 바꾸고 나머지는 고정해(OFAT), 각 기법이 성능에 주는 영향을 따로 떼어 본다.
...
전체 240문 기준. 공통 조건은 **조(條) 단위 청킹**이며, 하이브리드는 RRF 융합 · dense 임베딩 = KURE-v1 · 리랭커 = bge-reranker-v2-m3. (검색기·리랭커·증강만 바꿔 비교)
...
| 증강 ❌ | HyDE[[9]](#ref9)·HyPE[[10]](#ref10) 모두 **전 dense 기저에서 baseline 미달**(아래 그리드). 리랭커가 손상을 일부 복구하나 여전히 낮고, 둘을 겹치면 최악 |
