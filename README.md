...
HyDE·HyPE를 검색 구성 4종에 각각 얹어 봤지만 어디서도 도움이 되지 않는다.
...
- **리랭커는 손실을 줄일 뿐 되돌리지 못한다.** 벡터+HyDE에 리랭커를 더하면 0.652→0.758로 회복되지만 증강 전(0.808)에 못 미치고, 하이브리드+HyPE도 리랭커를 얹어야 증강 전 값(0.860)에 닿을 뿐 그 이상은 못 넘는다.
- **HyDE+HyPE를 겹치면 가장 나쁘다(0.562).** 두 증강을 함께 쓰면 단일 적용보다도 떨어진다.
- **하이브리드도 예외가 아니다.** HyPE를 얹으면(BM25는 원문, dense는 가설질문 임베딩으로 RRF) 0.831→0.762로 내려간다.
