...
**RAGAS 대응 (분류는 맞췄으나 라이브러리는 미사용).** RAGAS[[16]](#ref16)는 RAG 평가의 사실상 표준이다(정답 없이 LLM이 자동 채점). 본 연구 메트릭도 그 분류에 맞췄다. 다만 라이브러리를 쓰지 않고 직접 구현했는데, ① gold(정답·정답조문 식별자)로 레퍼런스 기반의 더 엄밀한 채점, ② 심사모델 계열 강제 분리, ③ 조문 식별자 인용검증과 한국어 프롬프트 직접 통제를 위해서다.
...
| citation F1 | (없음) | 조문 식별자 일치, 자동·도메인 특화 |

검색 평가의 recall@k·nDCG는 식별자 기반 결정론 채점이라 RAGAS의 LLM 판정 context precision/recall보다 객관적이다.
...
그래프 증강 모드가 모두 naive보다 낮다. 엔티티 그래프가 오히려 관련 청크 검색을 흐트러뜨린다. crossref(최고 0.467 vs BM25 0.750)·multihop에서도 기대한 이득이 없다. (LightRAG 자체 청킹 1200자 + 식별자 역매핑 근사로 일부 과소평가 가능하나 격차가 커 결론은 견고.)
