...
정보요구형 두 벤치마크(**Lexical · Semantic**)에서 모든 검색 구성을 **3개 메트릭(recall@5 · MRR · nDCG@10)** 으로 평가했다(평균 recall@5 순). 청킹 조(條) 단위·임베딩 KURE-v1 고정, 리랭커 4종 비교. 증강 전체 그리드는 [§3.1.2](#312-실험별-결과), 위치 지정(Locator) 질의는 성격이 달라 [§5](#5-locator-benchmark--조문-위치-질의)에서 별도로 다룬다.
...
> **그림 1.** 검색 구성별 Lexical·Semantic recall@5(막대 번호는 위 표의 #, 위가 평균 상위).

#### 3.1.2 실험별 결과

> **표 2.** 실험별 결과 요약 — 비교 축별 결론과 근거.
...
> **표 3.** 증강 그리드 — HyDE·HyPE를 dense 기저별로 적용한 recall@5 (Lexical).
...
> **표 4.** 답변모델 비교 — 5종 × 6개 메트릭 judge 점수 (Lexical).
...
> **그림 6.** 답변모델 5종 × 6개 메트릭 judge 점수 히트맵(진할수록 높음).
...
> **그림 7.** factoid 격식체·구어체 쌍의 검색기별 recall@5.
...
> **표 5.** Lexical vs Semantic 질문 예시 (동일 조문).
...
> **그림 8.** 동일 기법의 Lexical→Semantic recall@5 변화(점=벤치마크, 선 길이=변화 크기).

아래 표는 **Semantic recall@5 기준으로 정렬**해 그대로 Semantic 리더보드 역할을 한다(Lexical 열·Δ로 변화를 함께 본다).

> **표 6.** 검색 재평가 — Lexical→Semantic recall@5 변화 (Semantic 순).
...
> **표 9.** Locator 검색 결과 — 검색기별·유형별 recall@5.
