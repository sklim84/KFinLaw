...
**검증 — Semantic Benchmark.** 이 의심을 가리기 위해 두 번째 벤치마크를 만들었다. 같은 코퍼스·유형 분포에서 법령명·조문번호·고유 전문용어를 빼고 일상어로 풀어 묻게 생성하고, **검색기와 무관한 결정론적 필터**로 거른다 — 코퍼스 문자 4-gram의 문서빈도(DF)로 '희소 용어'를 판정해, 질문이 정답 조문의 희소 4-gram을 재사용하면 폐기한다(생성 시도의 31%가 누출로 폐기). BM25 일관성 필터를 쓰지 않아 어휘중첩 편향이 재유입되지 않는다.
