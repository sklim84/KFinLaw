...
위 코퍼스에서 평가용 **240문**을 반자동으로 만들었다. 생성모델 Mistral Small 4 (119B)[[27]](#ref27)가 각 조문·별표에서 Q&A를 생성하고, **일관성 필터**로 거른다. 이 필터는 생성된 질문을 다시 검색기에 넣어 원래 조문이 다시 검색되는 질문만 채택한다(round-trip BM25).

질문은 네 유형을 각 60문씩 균형 있게 두었다: 정의·요건(factoid), 교차참조(crossref), 별표 조회(byeolpyo), 본법·시행령 멀티홉(multihop). 정답 근거(gold)는 조문·별표 식별자로 고정했다. 멀티홉 60문은 본법과 시행령을 함께 짚어 정답이 2개씩이고 나머지는 1개씩이라, 정답 식별자는 모두 300개이며 전부 코퍼스에 존재함을 확인했다. 또 정의·요건(factoid)에는 같은 사실을 격식체와 구어체로 묻는 30쌍을 넣어 어휘격차를 짝지어 측정했다.

평가는 성격이 다른 세 벤치마크로 나눈다. 셋 다 동일한 골드셋 스키마를 따르며(구축 방법은 아래 펼침), 평가 결과는 [§2.4.1](#241-retrieval-evaluation)에서 다룬다.

- **Lexical Benchmark** (240문): 앞서 만든 240문으로, 조문 용어가 자연스럽게 섞여 어휘중첩이 크다. 검색기가 표면 어휘중첩에 얼마나 의존하는지 본다.
- **Semantic Benchmark** (240문): 같은 코퍼스·유형 분포에서 전문용어를 일상어로 풀어 어휘격차를 크게 만든 짝. 의미를 이해해야 풀리도록 설계했다.
- **Locator Benchmark** (103문, 결정론 생성): 사용자가 조 위치를 직접 지정하는 known-item 질의("○○법 제5조")다. 정보요구형(Lexical·Semantic)과 성격이 달라 따로 다룬다.
...
**(RQ4) 답변 품질은 모델 크기가 좌우하는가, 도메인 적합성이 좌우하는가?** 검색을 최적 구성(하이브리드+리랭커)으로 고정하고 답변모델만 바꿔, 검색이 찾은 근거 위에서 만든 답의 품질을 본다(Lexical Benchmark 240문, judge 점수 0~1).
