...
> - 📍 **위치 질의(Locator)는 RAG의 약점이다.** &nbsp; 최고 recall 0.66, 구조적 조회가 정답(목적2 근거). (표 5)
>
...
```bash
cd rag                                            # 목적1(RAG 벤치마크)은 rag/ 아래에서 실행
...
├── mcp/                           # 목적2: MCP 서버 (law.go.kr 라이브 API)
└── cli/                           # 목적2: CLI (MCP 도구 래핑)
```
...
> ✅ **구현 완료** (목적2 · 서버 [`mcp/`](mcp/) · 등록·전체 도구 명세 [mcp/README](mcp/README.md))
