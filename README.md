...
[**한눈에**](#한눈에) · [1. 개요](#1-개요) · [2. 벤치마크 설계](#2-벤치마크-설계) · [3. 결과](#3-결과) · [4. 참고사항](#4-참고사항) · [5. 실험 환경](#5-실험-환경) · [6. 데이터 파이프라인](#6-데이터-파이프라인) · [References](#references)
...
일변수 격리(OFAT), 질문유형·register(격식/구어)별 분해.
...
청킹·임베딩·검색기·리랭커·증강 기법을 바꿔가며 240문의 회수 성능을 측정했다. 종합 점수는 아래 리더보드에, 실험별 요점은 이어지는 표와 그림에 정리한다.
...
> **그림 2.** 청킹 단위 — 조(條) 단위가 양쪽 최고. &nbsp; **그림 3.** 검색기 × 질문유형 — factoid는 dense, crossref·별표는 BM25.
...
검색을 **최적 config(하이브리드+리랭커)로 고정**하고 답변모델만 바꿔 답의 품질을 본다. judge는 답변모델과 다른 계열(gpt-oss-120b, temp=0)이며, 인용 정확도만 자동·객관이다. 검색 config는 `config.yaml › answer_eval`에 고정하고, `--retrieval {good,bad,none}`로 검색품질 전이·closed-book 대조도 할 수 있다.
...
검색=하이브리드+리랭커 고정, 240문, judge 점수 0~1.
...
> **그림 8.** 동일 기법을 두 벤치마크로 평가한 recall@5 변화. BM25는 붕괴(0.835→0.510), dense 벡터는 강건(0.767→0.767)해 순위가 뒤바뀐다.

**결과 (recall@5).**
- **BM25는 붕괴, dense는 강건 → 순위 역전.** BM25 0.835→0.510(−39%), 벡터(KURE) 0.767→0.767(±0). 어휘격차가 큰 현실적 질의에선 dense가 BM25를 크게 앞선다. Lexical Benchmark의 BM25 우위는 상당 부분 어휘중첩 인공물이었다.
- **최적 조합이 뒤바뀐다.** Lexical에선 하이브리드+리랭커(0.860)가 최고지만, Semantic에선 **벡터+리랭커(0.777)** 가 최고이고 하이브리드+리랭커는 0.756으로 밀린다 — BM25 성분이 어휘격차에서 오히려 발목을 잡는다.
- **임베더 강건성 차이.** KoE5가 어휘격차에 가장 강건(0.710→0.756, +0.046), KURE는 최고 성능을 유지(±0), BGE-M3는 최저.
- **증강 기법의 부정 결과는 두 벤치마크 모두에서 견고하다.** HyPE(0.579)·HyDE(0.592)·LightRAG(mix 0.665)는 Semantic에서도 단순 벡터(0.767)에 못 미친다. 즉 "정교한 증강이 효과 없다"는 [결과 3.1](#31-검색-평가)의 결론은 골드셋 편향 탓이 아니라 견고한 결과다.
