...
**답변 메트릭**은 인용 F1만 자동·객관이고, 나머지 다섯 항목은 judge 루브릭으로 채점한다. judge(gpt-oss-120b[[28]](#ref28))는 답변모델과 다른 계열을 쓰고, 검색은 최적 구성(하이브리드+리랭커)으로 고정해 답변모델 효과만 본다. 생성·심사·답변 LLM을 서로 다른 계열로 나눈 것은, 같은 계열이면 심사기가 자기 쪽을 후하게 평가하는 편향이 생기기 때문이다. 모든 모델은 temp=0으로 실행한다.

| 메트릭 | 측정 | 채점 |
|---|---|---|
| answer correctness (정확성) | 정답과 일치 | judge (레퍼런스 기반: 정답+정답조문 제공) |
| faithfulness (충실성) | 답이 검색된 컨텍스트에 근거(환각 없음) | judge |
| answer relevancy (적합성) | 질문에 답하는가 | judge |
| completeness (완결성) | 요건/항목 누락 없는가 | judge |
| context utilization (맥락활용) | 검색된 컨텍스트를 실제 활용 | judge |
| citation F1 (인용F1) | 인용 조문이 gold_ids와 일치 | 자동 (LLM 불필요) |

<details>
<summary>RAGAS 대응 · 고유 비교 축 · 엄밀성</summary>

**RAGAS 대응 (분류는 맞췄으나 라이브러리는 미사용).** RAGAS[[16]](#ref16)는 RAG 평가의 사실상 표준이다(정답 없이 LLM이 자동 채점). 본 연구 메트릭도 그 분류에 맞췄다. 다만 라이브러리를 쓰지 않고 직접 구현했는데, ① gold(정답·정답조문 uid)로 레퍼런스 기반의 더 엄밀한 채점, ② judge 계열 강제 분리, ③ 조문 uid 인용검증과 한국어 프롬프트 직접 통제를 위해서다.
...
**엄밀성.** judge 독립성(답변모델과 다른 계열, 같으면 자기우대 +10~25%)과 레퍼런스 기반 채점(judge에 정답+정답조문 제공)은 이미 적용했다. 인간(법률가) 표본 검수 ~150문항과 일치계수(κ) 보고(ARES[[17]](#ref17)·KBL[[18]](#ref18) 관행)는 향후 과제다.
...
**(RQ4) 답변 품질은 모델 크기가 좌우하는가, 도메인 적합성이 좌우하는가?** 검색을 최적 구성(하이브리드+리랭커)으로 고정하고 답변모델만 바꿔가며, 검색이 찾아준 근거로 생성한 답의 품질을 평가한다(Lexical Benchmark 240문항, judge 점수 0~1).
