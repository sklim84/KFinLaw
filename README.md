...
작업 순서는 벤치마크(목적1, [§2](#2-kfinlaw-with-rag)) → MCP([§3](#3-kfinlaw-with-mcp))·CLI([§4](#4-kfinlaw-with-cli))다. 본 연구는 다섯 연구 질문(RQ1~5)을 각 실험 절에서 던지고 답하며, 종합 결론·권고는 [§5](#5-conclusion-and-recommendations)에 모은다.
...
| | 별표 MinerU[[19]](#ref19) | 병합셀이 복잡한 표에 더 정확한 레이아웃 파서(미연결, 비교만 수행) |
...
국가법령정보센터 Open API(<https://open.law.go.kr>)에서 법령 검색·본문(조문)·별표서식을 받는다. 인증키(OC)는 사용자별로 발급·설정한다(회원가입 후 OPEN API 신청; 발급·설정 절차는 [§2.5](#25-reproduction)).
...
별표(부록·표)는 PDF로 변환했다. API가 주는 HTML은 JS iframe이라 직접 스크래핑이 안 되고, 원본 HWP는 변환 과정에서 표 구조가 깨지기 때문이다. 변환기는 GPU가 필요 없고 라이선스가 자유로운 **kordoc**(PDF 모드)으로 1,083개를 전수 변환했다. 병합셀이 복잡한 표는 MinerU가 더 정확하지만 파이프라인에는 연결하지 않고 비교 평가만 수행했다(표의 *MinerU(미연결)*).
...
**(RQ5) 위치 질의를 RAG 검색으로 풀 수 있는가?** "○○법 제5조", "제5조부터 제8조까지"처럼 사용자가 **위치(법령명+조번호)를 이미 알고** 그 조문을 찾는 known-item 질의다. 정보요구형(Lexical·Semantic)과 달리 정답이 위치로 특정되며, 벡터 검색의 '정확 식별자' 처리력을 본다.
