...
**국가법령정보센터 연계 MCP/CLI를 구현**하는 KFTC 연구 과제(KA-013).
...
>
> - ✅ **검색은 하이브리드 + 리랭커가 최적이다.** &nbsp; Lexical recall@5 0.86(평균 0.81), 리랭커 적용이 성능을 가장 크게 좌우(종류 차이는 작음). (표 1)
> - ✅ **한국어 임베딩은 KURE-v1이 최선이다.** &nbsp; KoE5·BGE-M3보다 우위, BM25가 어휘 매칭을 보완해 하이브리드가 안정적. (표 1)
> - ❌ **증강(HyPE·HyDE·LightRAG)은 효과가 없다.** &nbsp; 모두 증강 전보다 낮아 "정교 ≠ 우위", 도입 전 실측이 필수. (그림 5)
> - ✅ **답변모델 품질은 크기로 예측되지 않는다.** &nbsp; 31B gemma-4가 100B(Solar)·67B(A.X)를 앞선다. 큰 모델이 곧 낫지는 않으니 실측 필수. (표 6)
> - 🔬 **어휘격차가 크면 최적이 벡터 + 리랭커로 뒤집힌다.** &nbsp; Semantic에서 BM25 붕괴(0.835→0.510), 벡터는 불변(0.767). (그림 6)
> - 📍 **위치 질의(Locator)는 RAG의 약점이다.** &nbsp; 최고 recall 0.66, 구조적 조회가 정답(구조적 조회 근거). (표 5)
>
...
| 2 | 국가법령정보센터 **MCP/CLI** (Claude 호환) | RAG 벤치마크 결과(특히 위치 질의 한계)에서 도출. `law.go.kr` 라이브 API(검색·조문·별표·인용검증) 기반, 경량·신선도 우선. **MCP·CLI 구현 완료**([§3](#3-kfinlaw-with-mcp)·[§4](#4-kfinlaw-with-cli)) |

> 본 보고서는 **벤치마크 결과**(§2)를 중심으로, 그 결과로 설계·구현한 **MCP/CLI**([§3](#3-kfinlaw-with-mcp)·[§4](#4-kfinlaw-with-cli))를 함께 다룬다.

작업 순서는 벤치마크([§2](#2-kfinlaw-with-rag))를 먼저 하고, 이어서 MCP([§3](#3-kfinlaw-with-mcp))·CLI([§4](#4-kfinlaw-with-cli))를 진행한다. 각 실험 절은 핵심 질문을 던지고 답하며, 종합 결론·권고는 [§5](#5-conclusion-and-recommendations)에 모은다.
...
![전체 평가 흐름](rag/benchmark/figures/fig_00_pipeline.png)

> **그림 1.** 전체 평가 흐름. 데이터(코퍼스·골드셋) → ① 검색 평가 → 최적 구성 고정 → ② 답변 평가.

### 2.1 Pipeline Components

파이프라인 단계별 선택지를 한 번에 하나씩만 바꾸며(일변수 격리) 비교했다. 아래 표에 단계별 비교 선택지를 정리한다(답변 생성은 답변모델 5종을 비교, [§2.5](#25-answer-evaluation)). 평가에 쓰는 생성모델(골드셋 생성, §2.2)·심사모델(judge, §2.3)은 파이프라인 구성요소가 아니라 각 해당 절에서 다룬다.
...
| | BGE-M3[[7]](#ref7) | 다국어·다기능·다양한 입력 길이를 지원하는 임베딩, KURE의 베이스 모델, 다국어 일반 성능 기준 |
...
> *임베딩* 3종은 모두 **1024차원**이고 XLM-RoBERTa-large 백본을 공유한다(컨텍스트는 KURE·BGE-M3 8192, KoE5 512). 차원이 같아 임베더 비교가 *차원 통제* 상태에서 이뤄진다. *재순위*는 리랭커 *적용 여부 자체*가 종류 선택보다 성능을 더 크게 좌우한다.
...
별표(부록·표)는 PDF로 변환했다. API가 제공하는 HTML은 JS iframe이라 직접 스크래핑할 수 없고, 원본 HWP는 변환 과정에서 표 구조가 깨지기 때문이다. 변환기는 **kordoc**(PDF 모드)으로 1,083개를 전수 변환했다.
...
> 본법 13 + 시행령 13 + 시행규칙 6 = **32개**. 전자금융·자본시장·은행·보험·여신·신용정보·예금보호·지배구조·실명거래 등 핵심 영역을 포괄. 전체 메타(mst·법령ID·조문수·별표수)는 `rag/benchmark/corpus_ids.json`.
...
- **Lexical Benchmark** (240문항): 생성모델 Mistral Small 4 (119B)[[27]](#ref27)가 각 조문·별표에서 Q&A를 만들고, **일관성 필터**로 거른다. 생성한 질문을 다시 검색기에 넣어 원래 조문이 검색되는 것만 채택하는 방식이다(round-trip BM25 · Promptagator[[14]](#ref14)). 정답이 식별자라 채점 편향과 무관해 오픈웨이트 생성모델로도 충분하다(InPars[[12]](#ref12)·InPars-v2[[13]](#ref13)). 정의·요건(factoid), 교차참조(crossref), 별표 조회(byeolpyo), 본법·시행령 멀티홉(multihop) 네 유형을 각 60문항씩 구성했고, 멀티홉은 정답이 2개씩이라 정답 식별자가 모두 300개다. factoid에는 같은 사실을 격식체·구어체로 묻는 30쌍도 포함했다. 질문이 조문 용어를 자연스럽게 써서 어휘중첩(질문이 조문과 같은 단어를 쓰는 정도)이 크다.
- **Semantic Benchmark** (240문항): Lexical과 같은 코퍼스·유형 분포를 쓰되, 전문용어를 일상어로 풀어 어휘격차를 크게 키웠다. 어휘중첩이 재유입되지 않도록 BM25 일관성 필터 대신 **결정론적 필터**를 쓴다. 코퍼스 문자 4-gram의 문서빈도로 희소 용어를 판정해, 질문이 정답 조문의 희소 4-gram을 다시 쓰면 버린다(생성 시도의 31%가 이 누출로 폐기됐다). 검색기가 표면 어휘중첩에 얼마나 의존하는지 가려낸다.
  ...
  | **Semantic** | **예금보호 관련 기금**을 갚기 위한 **특별 부담금**을 제때 못 내면, 그다음 날부터 얼마를 더 내야 하나요? |

  Lexical은 조문의 정식 용어(`예금보험기금채권상환특별기여금`·`부보금융회사`·`연체료`)를 그대로 써 BM25가 단어만 맞춰도 정답을 찾지만, Semantic은 같은 사실을 일상어(`예금보호 관련 기금`·`특별 부담금`)로 풀어, 의미를 이해하는 벡터 검색이라야 정답을 찾는다.
...
먼저 검색 평가를 최적화해 구성을 고정하고, 그 구성을 토대로 답변 생성 평가를 수행한다. 변수는 한 번에 하나씩만 바꾸고 나머지는 고정해(OFAT) 각 기법의 영향을 분리한다. 검색 결과는 질문유형(factoid·crossref·byeolpyo·multihop)별로 나눠 기법의 강·약점을 살핀다.

**검색 메트릭**은 검색된 청크의 식별자를 정답 식별자와 대조해 recall@k · precision@k · MRR · nDCG@k[[15]](#ref15)를 **결정론적으로** 계산하므로 LLM 판정이 개입하지 않는다.

**답변 메트릭**은 인용 F1만 자동·객관이고, 나머지 다섯 항목은 심사모델 루브릭으로 채점한다. 심사모델은 각 차원을 1점부터 5점까지 정수로 매기고 이를 (점수−1)/4로 [0,1] 정규화한다. 차원별 채점 기준은 아래 표와 같고, 심사 프롬프트 전문은 [`rag/benchmark/eval/answer_metrics.py`](rag/benchmark/eval/answer_metrics.py)(`JUDGE_SYS`)에 있다. 분류는 RAG 평가의 사실상 표준인 RAGAS[[16]](#ref16)에 맞추되, 정답·정답조문으로 더 엄밀하게 채점하고 심사모델 계열을 강제로 분리하며 조문 식별자 인용검증을 더하려고 라이브러리 대신 직접 구현했다. 심사모델(gpt-oss-120b[[28]](#ref28))은 답변모델과 다른 계열을 쓴다. 같은 계열로 두면 심사모델이 자기 쪽을 10~25% 후하게 평가하는 편향이 생기기 때문이다. 검색은 최적 구성(하이브리드+리랭커)으로 고정해 답변모델 효과만 보고, 모든 모델은 샘플링 온도 0(temperature=0)으로 실행한다.
...
청킹·임베딩·검색기·리랭커·증강을 바꿔가며 정보요구형 두 벤치마크(**Lexical · Semantic**)에서 검색 성능을 측정했다. 청킹은 조(條) 단위로, 임베딩은 주 실험에서 KURE-v1으로 고정하고 나머지를 변화시켰으며(임베더 강건성을 비교하는 그림 6에서만 KoE5·BGE-M3를 변수로 둔다), 모든 검색 구성을 recall@5 · MRR · nDCG@10으로 평가했다.
...
- **리랭커는 적용 여부가 성능을 크게 좌우하고, 어떤 리랭커를 쓰는지는 영향이 작다.** ko-reranker-8k는 Lexical 최고지만 Semantic이 급락해, 평균이 가장 균형 잡힌 bge-reranker-v2-m3를 기본 리랭커로 삼는다(🏆).

#### 2.4.2 Component Analysis

**청킹·임베딩·검색기 중 무엇이 검색 성능을 좌우하는가?** 한 번에 한 축만 바꿔(OFAT) 각 축이 성능에 얼마나 기여하는지 측정했다. 축별 결론과 근거는 표 2에, 각 축의 세부 비교는 그림 2~5에 있다.
...
| 검색기·리랭커 | **하이브리드 + 리랭커** 🏆 | 리랭커가 성능을 가장 크게 좌우 (0.831→0.860) |
...
| | |
|---|---|
| ![청킹 비교](rag/benchmark/figures/fig_02_chunking.png) | ![검색기×질문유형](rag/benchmark/figures/fig_03_by_type.png) |
| ![recall@k 곡선](rag/benchmark/figures/fig_04_recall_at_k.png) | ![증강 기법 부정 결과](rag/benchmark/figures/fig_05_augmentation.png) |

> **그림 2.** 청킹 단위별 recall@5. &nbsp; **그림 3.** 검색기 × 질문유형별 recall@5.
> **그림 4.** 컴포넌트 누적 recall@k. &nbsp; **그림 5.** 증강 적용 전후 recall@5.
...
**널리 쓰이는 증강기법은 실제로 도움이 되는가?** HyDE·HyPE를 검색 구성 4종에 각각 적용해 봤지만 어느 구성에서도 나아지지 않는다.
...
- **리랭커는 손실을 줄일 뿐 되돌리지 못한다.** 벡터+HyDE에 리랭커를 더하면 0.652→0.758로 회복되지만 증강 전(0.808)에 못 미치고, 하이브리드+HyPE도 리랭커를 더하면 증강 전(0.860) 수준에서 멈춘다.
- **HyDE와 HyPE를 함께 적용하면 가장 나쁘다(0.562).** 한쪽만 쓸 때보다도 더 떨어진다.
- **하이브리드도 예외가 아니다.** HyPE를 적용하면(BM25는 원문, 벡터는 가설질문 임베딩으로 RRF) 0.831→0.762로 내려간다.

**그래프 RAG(LightRAG)는 검색에 도움이 되는가?** 모드를 바꿔가며 평가했지만 어느 모드도 단순 벡터 검색을 넘지 못한다.
...
- **그래프 의존도가 높은 모드일수록 recall이 낮다(단순 벡터 0.738 → local 0.463).** 엔티티 그래프가 오히려 관련 청크 검색을 방해한다.
- **그래프가 유리할 것으로 예상한 유형에서도 이득이 없다.** 교차참조(최고 0.467 vs BM25 0.750)·멀티홉 모두 향상이 없다.
- **측정 한계가 있어도 결론은 견고하다.** LightRAG는 다른 기법처럼 조 단위로 자르지 않고 1200자 단위로 자체 청킹한다. 그래서 반환된 청크가 어느 조문에 해당하는지 정확히 가려내기 어려워, 채점 시 정답 조문과 대조하는 과정에서 맞은 답을 일부 놓쳐 recall이 실제보다 낮게 측정됐을 수 있다. 그래도 단순 벡터와의 격차가 커 결론은 바뀌지 않는다.

#### 2.4.3 Lexical-Gap Robustness

**검색기 우열이 질의 표현(어휘중첩 ↔ 어휘격차)에 따라 뒤바뀌는가?** 질문이 조문 용어를 그대로 쓰면 검색어와 조문이 겹치는 정도(어휘중첩)가 크고, 같은 뜻을 일상어로 바꿔 물으면 둘 사이의 거리(어휘격차)가 크다. §2.2의 Lexical Benchmark는 조문에서 질문을 만들고 BM25 일관성 필터로 거르므로 어휘중첩이 큰 쪽으로 치우친다. 그렇다면 BM25 우위나 'HyPE·HyDE가 효과 없다'는 결과도 이런 치우침 때문에 나온 것일 수 있다. 이것이 사실인지 확인하려고 어휘격차를 키운 **Semantic Benchmark**를 만들었다(생성·필터는 §2.2).

Semantic에서 같은 기법들을 다시 평가하니 검색기 간 우열이 뒤집힌다(그림 6).

<img src="rag/benchmark/figures/fig_06_lowoverlap.png" alt="Lexical vs Semantic 벤치마크 비교">

> **그림 6.** 동일 기법의 Lexical→Semantic recall@5 변화(점=벤치마크, 선 길이=변화 크기).

- **BM25는 붕괴(−39%)하고 벡터는 그대로라 순위가 뒤집힌다.** 어휘격차가 큰 현실적 질의에선 벡터가 BM25를 크게 앞선다. Lexical Benchmark에서 BM25가 앞선 것은 상당 부분 어휘중첩이 부풀린 결과다.
- **최적 조합이 뒤바뀐다.** Lexical 최적은 하이브리드+리랭커지만 Semantic 최적은 **벡터+리랭커**다. 하이브리드는 BM25 성분이 어휘격차에 약해 순위가 떨어진다.
- **임베더마다 강건성이 다르다.** KoE5는 어휘격차에서 임베더 중 유일하게 상승(+0.046)했으나 절대 recall은 여전히 KURE(0.767) 아래(0.756)다. KURE는 최고 성능을 그대로 유지(±0)해 어휘격차에 강하면서 성능도 가장 높다.
- **증강 부정 결과는 두 벤치마크 모두에서 견고하다.** HyPE·HyDE·LightRAG는 Semantic에서도 단순 벡터에 못 미친다. "정교한 증강이 효과 없다"는 결론은 골드셋 편향 탓이 아니다.
- **질의 표현에 따라 최적 검색기가 달라지므로 상황에 맞게 골라 써야 한다.** 구체적 권고는 [§5](#5-conclusion-and-recommendations)에 정리했다.

#### 2.4.4 Locator Queries

**법령명과 조번호로 특정 조문을 직접 지정하는 '위치 질의'를 RAG 검색으로 풀 수 있는가?** "○○법 제5조", "제5조부터 제8조까지"처럼 사용자가 **답이 아니라 조문 위치를 이미 알고** 찾는 known-item 질의다(유형 예시는 [§2.2.4](#224-골드셋)). 정보요구형(Lexical·Semantic)과 달리 정답이 내용이 아니라 위치로 정해지며, RAG 검색이 정확한 식별자를 얼마나 잘 다루는지 살펴본다.

> **표 5.** Locator 검색 결과, 검색기별·유형별 recall@5.
...
- **BM25가 가장 약하다(0.197).** 일반 IR 직관(정확한 식별자는 BM25가 강하다)과 반대다. 법령 코퍼스는 법령명이 그 법의 모든 조문에 반복되고 "제N조"가 교차참조로 곳곳에 나와, **식별자 토큰의 변별력이 떨어져** BM25가 정확한 조문을 짚어내지 못하는 것으로 추정된다(식별자 토큰 DF 분포 측정은 향후 과제).
...
- **범위 질의가 가장 어렵다**(멀티-gold, 최고 0.31). 연속된 여러 조문을 top-k 안에 모두 담기가 까다롭다.
- **전체 recall이 낮다**(최고 0.66). 위치 질의는 RAG 검색보다 **구조적 조회**(법령명과 조번호를 파싱해 조문을 바로 가져오는 방식)가 정답이며, 이것이 [MCP/CLI](#1-introduction) 구축의 직접적 근거다.

### 2.5 Answer Evaluation

**어떤 답변모델이 가장 좋은가? 큰 모델일수록 유리한가?** 검색을 최적 구성(하이브리드+리랭커)으로 고정하고 답변모델만 바꿔가며, 검색이 찾아준 근거로 생성한 답의 품질을 평가한다(Lexical Benchmark 240문항, 심사모델 점수 0~1).

> **표 6.** 답변모델 비교, 5종 × 6개 메트릭 심사모델 점수 (Lexical).
...
- **gemma-4-31B가 대부분의 메트릭에서 최상위다.** A.X-4.0(67B)이 바로 뒤를 잇고, 완결성만 A.X가 근소 우위다.
- **답변 품질은 모델 크기로 예측되지 않는다.** 같은 검색·프롬프트 조건에서 31B(gemma)가 100B(Solar)·67B(A.X)를 앞선다. 무작정 큰 모델이 낫지 않으니 후보를 실측 비교해 골라야 한다(검색 실험의 "정교 ≠ 우위"와 같은 맥락이다).

**어휘격차가 커져도 이 순위가 유지되는가?** 같은 답변 평가를 Semantic Benchmark에서 반복했다(검색은 그 최적인 벡터+리랭커로 고정).

> **표 7.** 답변 재평가, Lexical vs Semantic 평균 점수.
...
- **순위는 그대로다(gemma-4-31B 1위, A.X-4.0 2위).** 답변모델 선택은 질의 표현에 견고하다.
- **품질 하락은 소폭에 그친다(대부분 −0.03~0.06).** 검색 recall이 낮아진 만큼(0.860→0.777) 점수도 내려가지만 폭이 작아 파이프라인이 어휘격차에서도 안정적이다. 하락 폭은 Solar-100B가 가장 크다(−0.056).

> **한계.** 통제된 소규모 코퍼스(32법령)라 절대 수치보다 기법 간 상대 비교에 무게를 둔다. 답변 평가는 심사모델 기반이라 법률가 검수·일치계수(κ) 같은 인적 검증이 빠졌고(ARES[[17]](#ref17)·KBL[[18]](#ref18) 관행), 골드셋도 단일 생성모델(Mistral)에 의존한다. 보강 방향은 [§5](#5-conclusion-and-recommendations)에 정리했다.

### 2.6 Reproduction

벤치마크는 `rag/` 아래 `benchmark` 패키지다. `rag/`에서 `python -m benchmark.<모듈>`(데이터 수집은 `python scripts/<파일>.py`)로 실행하고, 모델은 전용 venv에서 `scripts/serve_model.sh {mistral|gptoss|stop}`로 띄운다. 버전 고정·서빙 플래그·디렉토리 구조는 아래 접은 글에 있다.

```bash
cd rag                                            # RAG 벤치마크는 rag/ 아래에서 실행
...
- 버전 고정: `rag/serving/requirements.venv.full.lock`(vllm 0.23.0 · mistral_common 1.11.3 · sentence-transformers 5.5.1 · KURE-v1). 시스템 vllm 0.20.1/mistral_common 1.11.2의 reasoning_effort·멀티모달 버그 회피.
...
│   ├── serving/  requirements.venv.lock / .full.lock   # 작동 버전 고정
│   ├── data/ (gitignore)  raw_xml · byeolpyo_pdf · byeolpyo_md
│   └── tools/ (gitignore) LibreOffice + H2Orestart
└── tool/                          # kfinlaw 패키지: MCP 서버 + CLI (law.go.kr 라이브 API)
    ├── kfinlaw_mcp/               # lawapi(코어) · server(MCP) · cli · finance_corpus
    ├── pyproject.toml             # 콘솔 스크립트 kfinlaw-mcp(서버) · kfinlaw(CLI)
...
> ✅ **구현 완료** (서버·CLI [`tool/`](tool/) · 등록·전체 도구 명세 [tool/README](tool/README.md))

**도입 근거.** [§2.4.4 Locator Queries](#244-locator-queries)에서 "○○법 제5조" 류 위치 질의는 RAG 검색의 최고 recall이 0.66에 그쳤다(BM25 0.20 · 하이브리드 0.44 · 하이브리드+리랭커 0.66). 법령명이 그 법의 모든 조문에 반복되고 "제N조"가 교차참조로 곳곳에 나와 식별자 토큰의 변별력이 낮기 때문이다. 이런 known-item 질의는 의미 검색이 아니라 **법령명과 조번호를 파싱해 조문을 바로 가져오는 구조적 조회**가 정확하다. 이것이 RAG가 아닌 라이브 API 연계를 택한 직접적 근거다.

**설계 원칙.** ① 위치 질의는 구조적 조회를 1차 경로로, 의미 질의는 §2의 하이브리드+리랭커를 보조로 결합한다(하이브리드 라우팅). ② 무거운 인덱싱 없이 라이브 API로 신선도를 우선한다. ③ 인용(교차참조) 검증을 핵심 기능으로 둔다.

**구현.** `law.go.kr` 라이브 API를 **9개 MCP 도구**로 노출한다(표준 라이브러리 + MCP SDK만 쓰는 경량 구현).
...
위치 질의는 `get_article`의 구조적 조회로 RAG의 약점(최고 recall 0.66)을 메운다. 배포는 **uvx**(`uvx kfinlaw-mcp`)·**Claude Code 플러그인**(`/plugin marketplace add sklim84/KFinLaw`)을 지원한다. 인증키(`LAW_OC`)·등록법·전체 명세는 [tool/README](tool/README.md)에 있다.

---

## 4. KFinLaw with CLI

> ✅ **구현 완료** (CLI는 [`tool/`](tool/) 패키지에 포함 · 사용법 [tool/README](tool/README.md))
...
CLI 구현은 MCP와 **한 패키지**(`tool/kfinlaw_mcp/cli.py`)라 한 번의 배포로 `kfinlaw`(CLI)·`kfinlaw-mcp`(서버)가 함께 설치된다. 전체 명령은 [tool/README](tool/README.md).
...
- **권장 기본 구성.** 조(條) 단위 청킹 + KURE-v1 임베딩 + **하이브리드(BM25+KURE) + 리랭커(bge-reranker-v2-m3)**. 두 벤치마크·전 메트릭에서 고르게 상위이며, 리랭커 *적용*이 성능을 가장 크게 좌우한다.
...
- **답변 모델.** 품질은 모델 크기로 예측되지 않는다. 31B gemma-4가 100B·67B급을 앞서므로, 무작정 큰 모델을 택하지 말고 후보 모델을 실측 비교해 고른다.
- **위치 질의는 RAG가 아니다.** "○○법 제5조" 류 known-item 질의는 RAG 검색이 약하다(최고 recall 0.66). 법령명과 조번호를 파싱해 **직접 조회(MCP/CLI)**하는 구조적 경로가 정답이며, 이것이 MCP/CLI의 근거다([§3](#3-kfinlaw-with-mcp)).

**다음 단계.** ① 법률가 표본 검수·일치계수(κ)로 답변 평가 신뢰성 보강, ② 다중 생성모델 골드셋 교차검증, ③ 시행일자 버전·no-answer 유형 확대.

---

## References

본 벤치마크가 채택·검증한 기법·모델·도구의 원천. 본문에서는 번호 링크(예: [[4]](#ref4))로 인용한다. [1]\~[18]은 기법 논문, [19]\~[21]은 리랭커, [22]\~[28]은 사용 LLM(모델 카드)이다.
