...
**파싱 — 별표(부록·표) 소스**
- **kordoc-md** : HWP·PDF 별표를 GPU 없이 마크다운 표로 변환(라이선스 자유) — 기본 변환기
- **평문** : 별표를 표 구조 없이 평문 텍스트로 — 표구조 보존 효과의 대조 기준
- **MinerU(미연결)** : 병합셀이 복잡한 표에 더 정확한 레이아웃 파서지만, 파이프라인에는 미연결하고 비교만 수행

**청킹 — 문서를 어떤 단위로 자를까**
- **조(article)** : 법령 조문 단위로 분할 — 법령의 자연 단위(기본값)
- **항(hang)** : 조 아래 항 단위로 더 잘게 — 세분화 효과 검증
- **고정토큰(fixed)** : 의미 경계를 무시하고 N토큰씩 균등 분할 — 일반 RAG 관행 대조
- **계층(parent-doc)** : 작은 청크로 검색하고 부모(조) 문맥을 반환 — small-to-big 방식

**임베딩 — 의미 벡터 모델** (셋 다 **1024차원** · XLM-RoBERTa-large 백본이라 차원이 통제된 비교)
- **KURE-v1**[[8]](#ref8) : 한국어 검색 특화 임베더, BGE-M3를 한국어로 파인튜닝 — 본 실험 기본 임베더(컨텍스트 8192)
- **BGE-M3**[[7]](#ref7) : 다국어·다기능·다입도(multi-granularity) 임베딩, KURE의 베이스 모델 — 다국어 일반 성능 기준(컨텍스트 8192)
- **KoE5**[[6]](#ref6) : multilingual-e5-large를 한국어로 파인튜닝 — E5 계열 대조(컨텍스트 512)

**검색기 — 어떻게 후보를 회수할까**
- **BM25**[[2]](#ref2) : 어휘 빈도 기반 희소(sparse) 검색 — 정확한 단어 일치에 강함
...
**재순위(리랭커) — 회수 후보를 크로스인코더로 재정렬** (적용 여부 자체가 가장 큰 레버)
- **bge-reranker-v2-m3**[[5]](#ref5) : BGE-M3 기반 다국어 크로스인코더 — 권장 기본값(균형 최상)
- **ko-reranker** : 한국어 특화 크로스인코더
- **ko-reranker-8k** : 한국어·긴 컨텍스트(8k) 크로스인코더
- **bge-reranker-large** : 영어 대형 크로스인코더 — 규모 대조군

**증강 — 질의·색인을 가설 생성으로 보강**
- **HyPE(색인측)**[[10]](#ref10) : 색인 시 각 청크의 가설 질문을 생성·임베딩해 질문↔질문으로 매칭
- **HyDE(질의측)**[[9]](#ref9) : 질의 시 가설 답변을 생성해 그 임베딩으로 검색

**답변 생성** — 컨텍스트 포맷·인용 지시를 변형(답변모델 5종은 아래 역할 표).
...
위 코퍼스에서 평가용 **240문**을 반자동으로 만들었다. 생성기 Mistral Small 4가 각 조문·별표에서 Q&A를 만들고, **일관성 필터**로 걸러 채택한다 — 생성된 질문을 검색기에 다시 넣어 원본 조문이 회수되는 질문만 남기는 방식이다(round-trip BM25).
...
리랭커가 손상을 크게 복구하지만(예: 벡터+HyDE에 리랭커 추가 시 0.652→0.758) 여전히 기저 미만이고, **HyDE+HyPE 중첩이 가장 나쁘다**(0.562). HyPE는 색인을 가설질문 임베딩으로 교체하는 구조라 BM25 성분이 있는 하이브리드와는 결합되지 않는다(—).
...
> **한계 (연구 전체).** 통제된 소규모 코퍼스(32법령)라 절대 수치보다 기법 간 상대 비교에 무게를 둔다. 답변 평가는 LLM judge 기반이라 법률가 표본 검수(κ)가, 골드셋은 단일 생성기(Mistral)라 다중 생성기 교차검증이 향후 과제이며, 시행일자 버전·no-answer 유형은 아직 포함하지 않았다.
...
**다음 단계.** ① 목적2 MCP/CLI 구현, ② 법률가 표본 검수·일치계수(κ)로 답변 평가 신뢰성 보강, ③ 다중 생성기 골드셋 교차검증, ④ 시행일자 버전·no-answer 유형 확대.
