...
- **증강(HyDE·HyPE·LightRAG)은 어느 것도 적용 전 기저(baseline)를 넘지 못한다.**
...
**청킹·임베딩·검색기 중 무엇이 검색 성능을 좌우하고, 인기 증강기법은 실제로 도움이 되는가?** 한 번에 한 축만 바꿔(OFAT) 각 축의 기여를 따로 측정했다. 축별 결론과 근거는 표 2에, 청킹·검색기·증강의 세부 비교는 그림 2~5에 담았다.
...
- **질문유형마다 더 강한 검색기가 다르다(그림 3).** 의미를 묻는 factoid는 벡터가, 법령명을 그대로 인용하는 crossref는 BM25가 앞선다.
