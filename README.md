...
골드셋은 성격이 다른 세 벤치마크로 이뤄진다. 기본은 코퍼스에서 만든 **Lexical Benchmark**(240문항)이고, 이를 변형해 **Semantic**과 **Locator**를 둔다. 셋 다 정답을 조문·별표 식별자로 고정하는 동일한 스키마를 따르며, 평가 결과는 [§2.4.1](#241-retrieval-evaluation)에서 다룬다(구축 방법은 아래 펼침).
