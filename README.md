...
그래프에 더 기대는 모드일수록 더 낮아(단순 벡터 모드 0.738 → local 0.463), 엔티티 그래프가 오히려 관련 청크 검색을 흐트러뜨린다. 교차참조(최고 0.467 vs BM25 0.750)·멀티홉에서도 기대한 이득이 없다. (LightRAG 자체 청킹 1200자 + 식별자 역매핑 근사로 일부 과소평가 가능하나 격차가 커 결론은 견고.)
