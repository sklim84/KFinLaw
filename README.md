...
리랭커가 단일 최대 레버이므로, 고정한 bge-reranker-v2-m3가 최선인지 하이브리드 기저에서 4종을 세 벤치마크로 비교했다.

| 리랭커 | Lexical | Semantic | Locator |
|---|:-:|:-:|:-:|
| **bge-reranker-v2-m3** (기준·multilingual) | 0.860 | 0.756 | 0.660 |
| ko-reranker (한국어) | 0.838 | **0.769** | 0.669 |
| ko-reranker-8k (한국어) | **0.863** | 0.708 | **0.677** |
| bge-reranker-large (영어 대형) | 0.798 | 0.729 | 0.666 |

어느 것도 전 벤치마크 1위는 아니고 차이도 작다(±0.01~0.02). 기준 bge-reranker-v2-m3가 **모든 축에서 견고**해 기본값으로 타당하며, 한국어 특화 리랭커가 어려운 셋(Semantic·Locator)에서 미세 우위, **영어 대형은 일관 최하**(답변 평가의 "크기보다 도메인 적합성"과 같은 교훈)다. 즉 리랭커 *적용 여부*가 큰 레버이고 리랭커 *선택*은 작은 레버다. (Semantic은 [§4](#4-어휘중첩-편향-검증-lexical-vs-semantic-benchmark), Locator는 [§5](#5-locator-benchmark--조문-위치-질의))
