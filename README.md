...
청킹·임베딩·검색기·리랭커·증강 기법을 바꿔가며 회수 성능을 측정했다 — **Lexical Benchmark** 240문 기준이며, **Semantic Benchmark**(일상어 질의)에서의 변화는 [§4](#4-어휘중첩-편향-검증-lexical-vs-semantic-benchmark)에서 다룬다. 종합 점수는 아래 리더보드에, 실험별 요점은 이어지는 표와 그림에 정리한다.

#### 3.1.1 종합 리더보드

핵심 검색 구성을 **세 벤치마크 × 3개 메트릭(recall@5 · MRR · nDCG@10)** 으로 평가했다(평균 recall@5 순). 청킹은 조(條) 단위, 임베딩은 KURE-v1 고정, 리랭커는 4종 비교(bge-m3=bge-reranker-v2-m3 · ko=ko-reranker · ko-8k=ko-reranker-8k · large=bge-reranker-large). 증강(HyDE·HyPE·LightRAG)은 전부 baseline 미달이라 [§3.1.2](#312-실험별-결과)에 따로 둔다.

| # | 검색기 | 리랭커 | **Lexical**<br>R@5 | MRR | nDCG | **Semantic**<br>R@5 | MRR | nDCG | **Locator**<br>R@5 | MRR | nDCG | 평균<br>R@5 |
|:-:|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| 1 🏆 | 하이브리드(BM25+KURE) | bge-m3 | 0.860 | 0.775 | 0.793 | 0.756 | 0.661 | 0.684 | 0.660 | 0.786 | 0.703 | **0.759** |
| 2 | 하이브리드(BM25+KURE) | ko | 0.838 | 0.729 | 0.754 | 0.769 | 0.672 | 0.691 | 0.669 | 0.833 | 0.728 | **0.758** |
| 3 | 하이브리드(BM25+KURE) | ko-8k | 0.863 | 0.785 | 0.804 | 0.708 | 0.608 | 0.637 | 0.677 | 0.847 | 0.749 | **0.749** |
| 4 | 벡터(KURE) | bge-m3 | 0.808 | 0.748 | 0.751 | 0.777 | 0.661 | 0.684 | 0.626 | 0.759 | 0.700 | **0.737** |
| 5 | 하이브리드(BM25+KURE) | large | 0.798 | 0.699 | 0.719 | 0.729 | 0.621 | 0.644 | 0.666 | 0.812 | 0.716 | **0.731** |
| 6 | 하이브리드(BM25+KURE) | — | 0.831 | 0.710 | 0.735 | 0.658 | 0.562 | 0.588 | 0.443 | 0.515 | 0.449 | **0.644** |
| 7 | 벡터(KURE) | — | 0.767 | 0.656 | 0.672 | 0.767 | 0.612 | 0.648 | 0.376 | 0.439 | 0.402 | **0.636** |
| 8 | BM25 | — | 0.835 | 0.707 | 0.741 | 0.510 | 0.397 | 0.430 | 0.197 | 0.259 | 0.198 | **0.514** |

> **하이브리드+리랭커가 세 벤치마크·전 메트릭에서 고르게 강하다.** BM25는 Lexical만 높고 Semantic·Locator에서 무너지며, ko-reranker-8k는 Lexical 최고지만 Semantic 급락 — 평균이 가장 균형 잡힌 **bge-reranker-v2-m3가 기본값(🏆)**. Locator는 MRR(첫 정답 순위)이 recall@5보다 높은데, 첫 조문은 상위에 오지만 범위·모호 질의의 다중 정답을 다 담지 못한다는 뜻이다.

<img src="benchmark/figures/fig_01_leaderboard.png" width="700" alt="검색 기법 리더보드">

> **그림 1.** 핵심 검색 구성의 세 벤치마크 recall@5(위가 평균 상위). BM25는 Lexical만 높고 Semantic·Locator에서 급락, 하이브리드+리랭커는 셋 다 고르다.
...
| LightRAG | **효과 없음** ❌ | naive 0.738 < 단일 벡터 0.767, 그래프 모드는 더 낮음 |
