...
별표(부록·표)는 PDF로 변환했다. API가 주는 HTML은 JS iframe이라 직접 스크래핑이 안 되고, 원본 HWP는 변환 과정에서 표 구조가 깨지기 때문이다. 변환기는 **kordoc**(PDF 모드)으로 1,083개를 전수 변환했다. 병합셀이 복잡한 표는 MinerU가 더 정확하지만 파이프라인에는 연결하지 않고 비교 평가만 수행했다(표의 *MinerU(미연결)*).
