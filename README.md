...
리랭커가 손상을 크게 복구하지만(예: 벡터+HyDE에 리랭커 추가 시 0.652→0.758) 여전히 기저 미만이고, **HyDE+HyPE 중첩이 가장 나쁘다**(0.562). HyPE는 dense 색인을 가설질문 임베딩으로 통째로 바꾸는 방식이라 본 벤치마크에선 하이브리드와 결합해 돌리지 않았다(표의 "−"는 미평가). 원리상으로는 BM25는 원문, dense는 가설질문 임베딩으로 RRF 결합이 가능하다.
